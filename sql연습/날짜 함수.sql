...
-- 실행시작시간, 호출되는 시간
select now(), sleep(2), now(), sysdate() from dual;
...
select first_name, hire_date, period_diff(date_format(now(), '%y%m'),date_format(hire_date, '%y%m')) as "years of service" from employees;

-- date_add(adddate), date_sub(subdate)
-- 날짜를 date 타입의 컬럼이나 값에(여기서는 hire_date) type(year, month, day)의 표현식으로 더하거나 뺄 수 있다.
-- 예) 각 사원들의 근속 년 수가 5년이 되는 날에 휴가를 보내준다면 각 사원들의 근속 휴가 날짜
...
-- type
-- 문자 : varchar(갯수), char(갯수), text, CLOB
-- 정수 : tiny, medium, signed(int, intger), unsigned
-- 실수 : float, double
-- 시간 : date, datetime
-- LOB : CLOB(Char Large OBject), BLOB(Binary Large OBject)
